...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.NewAPIError) {
	...
	if info.IsStream {
		usage, err = xunfeiStreamHandler(c, *a.request, splits[0], splits[1], splits[2], info.ChannelSetting.Proxy)
	} else {
		usage, err = xunfeiHandler(c, *a.request, splits[0], splits[1], splits[2], info.ChannelSetting.Proxy)
	}
	return
}
